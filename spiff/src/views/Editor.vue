<template>
	<div class="h-full flex flex-col">
		...
		<Dialog v-model="showNotificationDialog" :options="{ title: 'Link Notification', size: '5xl' }">
			<template #body-content>
				<div class="space-y-4">
					<!-- Mode Tabs -->
					<div class="flex border-b border-gray-200">
						<button
							@click="notifDialogMode = 'select'"
							:class="[
								'px-4 py-2 text-sm font-medium border-b-2 transition-colors -mb-px',
								notifDialogMode === 'select'
									? 'border-blue-500 text-blue-600'
									: 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
							]"
						>
						...
						<button
							@click="notifDialogMode = 'create'"
							:class="[
								'px-4 py-2 text-sm font-medium border-b-2 transition-colors -mb-px',
								notifDialogMode === 'create'
									? 'border-blue-500 text-blue-600'
									: 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
							]"
						>
					...
					<!-- ========== Select Existing Mode ========== -->
					<div v-if="notifDialogMode === 'select'" class="space-y-3">
						<div class="text-sm text-gray-500">
							Search and select an existing Notification to link.
						</div>
						<!-- Search + DocType Filter row -->
						<div class="grid grid-cols-2 gap-3">
							...
							<div class="relative">
								<input
									...
									@focus="showNotifDoctypeDropdown = true"
									@blur="setTimeout(() => showNotifDoctypeDropdown = false, 200)"
								/>
								<button
									v-if="notifDoctypeFilter"
									@click="notifDoctypeFilter = ''"
									class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
								>
									<Icon icon="lucide:x" class="w-4 h-4" />
								</button>
								<div v-if="showNotifDoctypeDropdown && filteredNotifDoctypeOptions.length > 0" class="absolute z-50 w-full mt-1 max-h-48 overflow-y-auto bg-white border border-gray-200 rounded-md shadow-lg">
									<div
										v-for="dt in filteredNotifDoctypeOptions"
										:key="dt"
										@mousedown.prevent="notifDoctypeFilter = dt; showNotifDoctypeDropdown = false"
										class="px-3 py-1.5 text-sm cursor-pointer hover:bg-blue-50 text-gray-900"
									>{{ dt }}</div>
								</div>
							</div>
						...
						<div class="max-h-72 overflow-y-auto border border-gray-200 rounded-lg">
							...
							<div v-else-if="filteredNotifications.length === 0" class="p-6 text-center text-gray-400">
							...
							<div
								...
								:key="notif.name"
								@click="selectedNotification = notif.name"
								:class="[
									'flex items-center justify-between px-4 py-3 cursor-pointer border-b border-gray-100 last:border-b-0 transition-colors',
							...
							>
								<div>
									<div class="text-sm font-medium text-gray-900">
										<span class="mr-1.5">{{ channelIcon(notif.channel) }}</span>
										{{ notif.name }}
									</div>
									<div class="text-xs text-gray-500 mt-0.5">
										{{ notif.channel }}
								...
								<div class="flex items-center gap-2">
									...
									<Icon v-if="selectedNotification === notif.name" icon="lucide:check-circle" class="w-5 h-5 text-blue-500" />
								</div>
							</div>
						</div>
					</div>
					...
					<!-- ========== Create New Mode ========== -->
					<div v-else-if="notifDialogMode === 'create'" class="space-y-4">
						<div class="text-sm text-gray-500">
						...
						<!-- Row 2: Event + Document Type -->
						<div class="grid grid-cols-2 gap-4">
							<div>
								<label class="block text-xs font-medium text-gray-700 mb-1">Send Alert On <span class="text-red-500">*</span></label>
								<select
									v-model="newNotif.event"
									class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400"
								>
									<option value="">Select event...</option>
									<option v-for="evt in notifEvents" :key="evt" :value="evt">{{ evt }}</option>
								</select>
							</div>
							<div>
								...
								<div class="relative">
									<input
										v-model="notifNewDoctypeSearch"
										type="text"
										:placeholder="newNotif.document_type || 'Search DocType...'"
										class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400"
										@focus="showNotifNewDoctypeDropdown = true; notifNewDoctypeSearch = ''"
										@blur="setTimeout(() => showNotifNewDoctypeDropdown = false, 200)"
									/>
									<div v-if="showNotifNewDoctypeDropdown && filteredNotifNewDoctypeOptions.length > 0" class="absolute z-50 w-full mt-1 max-h-48 overflow-y-auto bg-white border border-gray-200 rounded-md shadow-lg">
										<div
											v-for="dt in filteredNotifNewDoctypeOptions"
											:key="dt"
											@mousedown.prevent="newNotif.document_type = dt; notifNewDoctypeSearch = dt; showNotifNewDoctypeDropdown = false"
											class="px-3 py-1.5 text-sm cursor-pointer hover:bg-blue-50 text-gray-900"
										>{{ dt }}</div>
									</div>
								</div>
						...
						<div v-if="newNotif.event === 'Method'" class="grid grid-cols-2 gap-4">
							<div>
								<label class="block text-xs font-medium text-gray-700 mb-1">Trigger Method</label>
								<input v-model="newNotif.method" type="text" placeholder="e.g. my_module.my_method" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
							</div>
						</div>
						<div v-if="['Days After', 'Days Before'].includes(newNotif.event)" class="grid grid-cols-2 gap-4">
							<div>
								<label class="block text-xs font-medium text-gray-700 mb-1">Reference Date</label>
								<input v-model="newNotif.date_changed" type="text" placeholder="e.g. due_date" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
							</div>
							<div>
								<label class="block text-xs font-medium text-gray-700 mb-1">Days Before or After</label>
								<input v-model="newNotif.days_in_advance" type="number" min="0" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
							</div>
						</div>
						<div v-if="newNotif.event === 'Value Change'" class="grid grid-cols-2 gap-4">
							<div>
								<label class="block text-xs font-medium text-gray-700 mb-1">Value Changed</label>
								<input v-model="newNotif.value_changed" type="text" placeholder="e.g. status" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
							</div>
						</div>
						...
						<div>
							...
							<input v-model="newNotif.subject" type="text" placeholder="e.g. New {{ doc.doctype }} Created" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
						</div>

						<!-- Condition -->
						<div>
							<label class="block text-xs font-medium text-gray-700 mb-1">Condition</label>
							<input v-model="newNotif.condition" type="text" placeholder='e.g. doc.status=="Open"' class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
						</div>

						<!-- Channel-specific fields -->
						<div v-if="newNotif.channel === 'Email'" class="grid grid-cols-2 gap-4">
							<div>
								<label class="block text-xs font-medium text-gray-700 mb-1">Sender (Email Account)</label>
								<input v-model="newNotif.sender" type="text" placeholder="Email Account name" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
							</div>
							<div>
								<label class="block text-xs font-medium text-gray-700 mb-1">Sender Email</label>
								<input v-model="newNotif.sender_email" type="email" placeholder="noreply@example.com" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
							</div>
						</div>
						<div v-if="newNotif.channel === 'Slack'">
							<label class="block text-xs font-medium text-gray-700 mb-1">Slack Webhook URL</label>
							<input v-model="newNotif.slack_webhook_url" type="text" placeholder="Slack Webhook URL record name" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
						</div>
						<div v-if="newNotif.channel === 'WhatsApp'">
							<label class="block text-xs font-medium text-gray-700 mb-1">Twilio Number (Communication Medium)</label>
							<input v-model="newNotif.twilio_number" type="text" placeholder="Communication Medium record name" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
						</div>
						<div v-if="newNotif.channel === 'Email'" class="flex items-center gap-4">
							<label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
								<input type="checkbox" v-model="newNotif.send_system_notification" class="rounded border-gray-300" />
						...
						<div class="border border-gray-200 rounded-lg p-3 space-y-3">
							<div class="flex items-center justify-between">
								...
								<button @click="addRecipientRow" type="button" class="text-xs text-blue-600 hover:text-blue-700 font-medium">+ Add Row</button>
							</div>
							<label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
								<input type="checkbox" v-model="newNotif.send_to_all_assignees" class="rounded border-gray-300" />
								Send To All Assignees
							</label>
							<div v-for="(row, idx) in newNotif.recipients" :key="idx" class="grid grid-cols-6 gap-2 items-end border-t border-gray-100 pt-2">
								<div>
								...
								<div class="flex items-end">
									<button @click="removeRecipientRow(idx)" type="button" class="p-1 text-red-400 hover:text-red-600 transition-colors" title="Remove row">
						...
						<div class="grid grid-cols-4 gap-4">
							<div class="col-span-1">
								...
								<select v-model="newNotif.message_type" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400">
									<option value="Markdown">Markdown</option>
									<option value="HTML">HTML</option>
									<option value="Plain Text">Plain Text</option>
								</select>
							</div>
							<div class="col-span-1" v-if="newNotif.channel === 'Email'">
								<label class="block text-xs font-medium text-gray-700 mb-1">Module</label>
								<div class="relative">
									<input
										v-model="notifModuleSearch"
										type="text"
										:placeholder="newNotif.module || 'Search Module...'"
										class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400"
										@focus="showNotifModuleDropdown = true; notifModuleSearch = ''"
										@blur="setTimeout(() => showNotifModuleDropdown = false, 200)"
									/>
									<div v-if="showNotifModuleDropdown && filteredNotifModuleOptions.length > 0" class="absolute z-50 w-full mt-1 max-h-48 overflow-y-auto bg-white border border-gray-200 rounded-md shadow-lg">
										<div
											v-for="mod in filteredNotifModuleOptions"
											:key="mod"
											@mousedown.prevent="newNotif.module = mod; notifModuleSearch = mod; showNotifModuleDropdown = false"
											class="px-3 py-1.5 text-sm cursor-pointer hover:bg-blue-50 text-gray-900"
										>{{ mod }}</div>
									</div>
								</div>
						...
						<div>
							...
							<textarea
								...
								class="w-full h-32 p-3 font-mono text-sm border border-gray-300 rounded-lg bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 resize-y"
								placeholder="Enter message template (Jinja supported)..."
								spellcheck="false"
							></textarea>
						</div>

						<!-- Print Settings (Email only) -->
						<div v-if="newNotif.channel === 'Email'" class="grid grid-cols-2 gap-4">
							<label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
								<input type="checkbox" v-model="newNotif.attach_print" class="rounded border-gray-300" />
								Attach Print
							</label>
							<div v-if="newNotif.attach_print">
								<label class="block text-xs font-medium text-gray-700 mb-1">Print Format</label>
								<input v-model="newNotif.print_format" type="text" placeholder="Print Format name" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
							</div>
						</div>
						...
						<div class="grid grid-cols-2 gap-4">
							<div>
								...
								<input v-model="newNotif.set_property_after_alert" type="text" placeholder="e.g. notification_sent" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
							</div>
							<div v-if="newNotif.set_property_after_alert">
								<label class="block text-xs font-medium text-gray-700 mb-1">Value To Be Set</label>
								<input v-model="newNotif.property_value" type="text" placeholder="e.g. 1" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
							</div>
						</div>
					</div>
				</div>
			...
			<template #actions>
				<div class="flex gap-2">
					<Button variant="subtle" @click="showNotificationDialog = false">Cancel</Button>
					<Button
						v-if="notifDialogMode === 'select'"
						variant="solid"
						@click="linkNotification"
						:disabled="!selectedNotification"
					>Link Notification</Button>
					<Button
						v-else
						variant="solid"
						@click="createAndLinkNotification"
						:loading="creatingNotification"
						:disabled="!newNotif.name || !newNotif.channel || !newNotif.document_type"
					>Create & Link</Button>
				</div>
			</template>
		</Dialog>
...
// Notification Dialog state (Send Task)
const showNotificationDialog = ref(false);
const notifDialogMode = ref("select"); // 'select' or 'create'
...
// Computed: DocType options for notification filter dropdown
...
// Computed: DocType options for Create New form
...
// Computed: Module options for Create New form
...
async function onLaunchNotificationEditor(event) {
	...
	// Fetch DocTypes and Modules for create form dropdowns (reuse cached)
	if (doctypeOptions.value.length === 0) {
		try {
			const dtResp = await frappeRequest({
				url: "/api/method/frappe.client.get_list",
...
function linkNotification() {
	// "Select Existing" mode — write the selected Notification name to BPMN element
...
async function createAndLinkNotification() {
	if (!newNotif.value.name || !newNotif.value.channel || !newNotif.value.document_type) {
		showNotification("Validation", "Name, channel, and document type are required.", "red");
		return;
	}
	...
	try {
		...
		// Write the new notification name back to the BPMN element
		...
		showNotification("Success", `Notification "${result.name}" created and linked.`, "green");
		showNotificationDialog.value = false;
		activeNotificationEvent = null;
	} catch (error) {
