				...
				<div class="grid grid-cols-1 xl:grid-cols-4 gap-6 items-start">
					...
					<div class="xl:col-span-3 space-y-6 flex flex-col">
						...
						<div class="bg-white rounded-lg shadow-sm border flex flex-col relative overflow-hidden">
							...
							<div class="relative w-full h-[600px] bg-slate-50">
							...
							<div class="px-5 py-2 border-t bg-white shrink-0 flex items-center justify-center gap-5 text-[11px] text-gray-600 shadow-inner z-10 relative">
								...
								<div class="w-px h-3 bg-gray-200"></div>
								<div class="flex items-center gap-1 px-2 py-0.5 rounded">
									<span class="font-bold text-gray-400 uppercase tracking-[0.05em] text-[9px]">Heat:</span>
									<div class="flex gap-0.5 items-center">
										<div class="w-2.5 h-2.5 rounded-sm bg-[#dcfce7] border border-[#16a34a]" title="1 pass"></div>
										<div class="w-2.5 h-2.5 rounded-sm bg-[#fef9c3] border border-[#ca8a04]" title="Low loop"></div>
										<div class="w-2.5 h-2.5 rounded-sm bg-[#fed7aa] border border-[#ea580c]" title="Medium loop"></div>
										<div class="w-2.5 h-2.5 rounded-sm bg-[#fecaca] border border-[#dc2626]" title="Hot loop"></div>
									</div>
								</div>
							</div>
						</div>
					...
					<div class="xl:col-span-1">
						<div class="bg-white rounded-lg shadow-sm border flex flex-col sticky top-6 overflow-hidden max-h-[850px]">
							...
							<div class="overflow-y-auto flex-1 p-0 custom-scrollbar bg-gray-50/20">
								<div v-if="groupedVariables.length > 0" class="p-2.5 space-y-2.5">
									<div v-for="group in groupedVariables" :key="group.label" class="rounded-lg border border-gray-200 bg-white overflow-hidden shadow-sm">
										<!-- Group header -->
										<div class="px-3 py-1.5 border-b bg-gray-50/80 flex items-center gap-1.5">
											<Icon :icon="group.icon" class="w-3.5 h-3.5" :class="group.iconColor" />
											<span class="text-[11px] font-bold uppercase tracking-wide" :class="group.labelColor">{{ group.label }}</span>
											<span class="text-[10px] text-gray-300 ml-auto font-mono">{{ Object.keys(group.vars).length }}</span>
										</div>
										<!-- Variables list -->
										<div class="divide-y divide-gray-100">
											<div v-for="(value, key) in group.vars" :key="key" class="px-3 py-1.5 flex items-start gap-2 hover:bg-blue-50/40 transition-colors">
												<span class="text-[11px] font-semibold text-gray-500 uppercase tracking-wide shrink-0 pt-px min-w-0 break-all leading-tight" style="max-width: 40%">{{ key }}</span>
												<span class="text-[13px] font-mono text-gray-800 break-all leading-tight min-w-0 whitespace-pre-wrap">{{ typeof value === 'object' ? JSON.stringify(value, null, 2) : value }}</span>
											</div>
										</div>
									</div>
								</div>
...
const processVariables = computed(() => {
	...
	try {
		...
		// SpiffWorkflow stores variables per-task; the top-level state.data
		// is emptied by clean_doc_from_wf_data() before serialization.
		// Extract from the best available task in priority order:
		//   1. Active task (READY, state=16) — current process position
		//   2. last_task pointer — last task the engine processed
		//   3. Most recently state-changed COMPLETED task (state=64)
		...
		// 1. Active (READY) task
		const activeTask = taskList.find(t => t.state === 16 && t.data && Object.keys(t.data).length > 0)
		if (activeTask) raw = activeTask.data

		// 2. last_task pointer
		if (!raw && state.last_task && tasks[state.last_task]?.data && Object.keys(tasks[state.last_task].data).length > 0) {
			raw = tasks[state.last_task].data
		}

		// 3. Most recently completed task with data
		if (!raw) {
			const completed = taskList
				.filter(t => t.state === 64 && t.data && Object.keys(t.data).length > 0)
				.sort((a, b) => (b.last_state_change || 0) - (a.last_state_change || 0))
			if (completed.length > 0) raw = completed[0].data
		}

		if (!raw) return {}

		// Filter out noise: null values and Frappe layout fields
		const filtered = {}
		for (const [key, value] of Object.entries(raw)) {
			// Skip null/undefined/empty values
			if (value === null || value === undefined || value === '') continue
			// Skip Frappe layout fields (Section Break, Column Break, Tab Break)
			if (/(^|_)(section|column|tab)(_break|_break_|$)/.test(key)) continue
			filtered[key] = value
		}
		return filtered
	} catch (e) {
...
// Group filtered process variables into logical categories for better readability
const groupedVariables = computed(() => {
	const vars = processVariables.value
	if (!vars || Object.keys(vars).length === 0) return []

	// Keys that belong to the "Process Context" group (engine-injected)
	const contextKeys = new Set([
		'context_doctype', 'context_docname',
		'triggered_by', 'trigger_doctype', 'trigger_docname',
		'docstatus'
	])
	// Keys that belong to the "User Actions" group (transient action data)
	...
	for (const [key, value] of Object.entries(vars)) {
		if (contextKeys.has(key)) context[key] = value
		else if (actionKeys.has(key)) actions[key] = value
		else docFields[key] = value
	}
...
function injectStyles() {
	if (!document.getElementById("bpmn-marker-styles")) {
		...
		style.innerHTML = `
			...
			/* Heatmap intensity levels (overrides highlight-done fill) */
			.heatmap-1:not(.djs-connection) .djs-visual > :nth-child(1) {
				fill: #dcfce7 !important; stroke: #16a34a !important; stroke-width: 2px !important;
			}
			.heatmap-2:not(.djs-connection) .djs-visual > :nth-child(1) {
				fill: #fef9c3 !important; stroke: #ca8a04 !important; stroke-width: 2px !important;
			}
			.heatmap-3:not(.djs-connection) .djs-visual > :nth-child(1) {
				fill: #fed7aa !important; stroke: #ea580c !important; stroke-width: 2px !important;
			}
			.heatmap-4:not(.djs-connection) .djs-visual > :nth-child(1) {
				fill: #fecaca !important; stroke: #dc2626 !important; stroke-width: 2.5px !important;
			}
			/* Hot flow — thicker stroke for frequently traversed sequence flows */
...
async function initViewer() {
	...
	if (!viewer.value) {
		...
		// Optional: Clear popup overlay if user clicks background
		viewer.value.get("eventBus").on("canvas.click", () => {
			viewer.value.get("overlays").remove({ type: "element-popup" });
		})
	}
...
function applyHighlights() {
	...
	try {
		...
		// Clear previous heatmap overlays (keep click-popups separate)
		overlays.remove({ type: "heatmap-badge" });

		let completedBpmnIds = new Set();
		let activeBpmnIds = new Set();
		// Frequency map: bpmn_id → count of COMPLETED task instances
		const frequencyMap = {};
		...
		if (details.value && details.value.workflow_state) {
			try {
				...
				for (const [uuid, taskData] of Object.entries(tasks)) {
					...
					} else if (state === 16 || state === 32 || state === 8) {
		...
		// Fallback: count completed from logs too
		logs.value
			.filter(l => l.action === "Completed")
			.forEach(l => {
				if (l.bpmn_id) {
					completedBpmnIds.add(l.bpmn_id);
				}
			});
		...
		// Determine max frequency for scaling (ignore 1-pass nodes)
		const maxFreq = Math.max(1, ...Object.values(frequencyMap));

		// Apply heatmap markers to completed BPMN elements
		completedBpmnIds.forEach(bpmnId => {
			try {
				const count = frequencyMap[bpmnId] || 1;
				if (count > 1 && maxFreq > 1) {
					// Multi-pass: apply heatmap gradient level 1-4
					const ratio = (count - 1) / (maxFreq - 1); // 0..1
					const level = Math.min(4, Math.max(1, Math.ceil(ratio * 4)));
					canvas.addMarker(bpmnId, `heatmap-${level}`);

					// Add count badge overlay
					const badgeClass = level >= 4 ? 'hot' : level >= 3 ? 'warm' : '';
					const badgeHtml = document.createElement("div");
					badgeHtml.className = `heatmap-badge ${badgeClass}`;
					badgeHtml.textContent = `×${count}`;

					overlays.add(bpmnId, "heatmap-badge", {
						position: { top: -10, right: -10 },
						html: badgeHtml,
					});
				} else {
					// Single pass: standard green
					canvas.addMarker(bpmnId, "highlight-done");
				}
			} catch(e) { console.warn('[BPMN] addMarker failed for', bpmnId, e); }
		});
		...
		if (elementRegistry) {
				...
				try {
					if (element.type === "bpmn:StartEvent") {
						...
						if (targetReached || (sourceDone && flow.target && flow.target.type.includes("EndEvent"))) {
							// If both source and target are hot (multi-pass), use thicker flow
							const srcFreq = frequencyMap[sourceId] || 0;
							const tgtFreq = frequencyMap[targetId] || 0;
							if (srcFreq > 1 && tgtFreq > 1) {
								canvas.addMarker(flow.id, "highlight-flow-hot");
							} else {
			...
			// Gateway highlights — also frequency-aware
			elementRegistry.filter(e => 
				e.type && e.type.includes("Gateway")
			).forEach(gateway => {
				try {
					const freq = frequencyMap[gateway.id] || 0;
					if (completedBpmnIds.has(gateway.id)) {
						if (freq > 1 && maxFreq > 1) {
							const ratio = (freq - 1) / (maxFreq - 1);
							const level = Math.min(4, Math.max(1, Math.ceil(ratio * 4)));
							canvas.addMarker(gateway.id, `heatmap-${level}`);
							const badgeClass = level >= 4 ? 'hot' : level >= 3 ? 'warm' : '';
							const bh = document.createElement("div");
							bh.className = `heatmap-badge ${badgeClass}`;
							bh.textContent = `×${freq}`;
							overlays.add(gateway.id, "heatmap-badge", {
								position: { top: -10, right: -10 }, html: bh,
							});
						} else {
...
function onElementClick(e) {
		...
		try {
			...
			for (const [uuid, taskData] of Object.entries(tasks)) {
				if (taskData.task_spec === elementId) {
					...
					if (taskData.state === 64) traversalCount++;
				}
			}
		} catch(e) {}
	}
	...
	const overlays = viewer.value.get("overlays");
	// Only clear click-popup overlays, preserve heatmap badges
	overlays.remove({ type: "element-popup" });
	...
	if (activeTask || doneLogs.length > 0) {
		...
		let metaHtml = "";
		if (traversalCount > 1) {
			metaHtml += `<div class="text-xs font-semibold text-orange-700 mt-1 bg-orange-50 px-1.5 py-0.5 rounded border border-orange-200 inline-block">⟳ Traversed ${traversalCount}× (loop)</div>`;
		}
		if (isDone) {
		...
		overlays.add(elementId, "element-popup", {
