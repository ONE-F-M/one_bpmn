					...
					>
						...
						<td class="px-3 py-2 text-right whitespace-nowrap">
							<Button variant="ghost" size="sm" @click.stop="copyLink(c)">
								{{ copied === c.name ? "Copied" : "Copy link" }}
							</Button>
						</td>
					</tr>
				</tbody>
...
async function copyLink(conversation) {
	try {
		const link = `${window.location.origin}/app/processa/sessions/${conversation.name}`;
		await navigator.clipboard.writeText(link);
		copied.value = conversation.name;
		setTimeout(() => (copied.value = ""), 1500);
	} catch (e) {
		error.value = "Could not copy the link — try copying it manually from the conversation detail.";
	}
}
