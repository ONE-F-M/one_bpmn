...
"""
A property-panel edit is recorded in the map's version history and nowhere else.
...
holding the bpmn_xml before and after — and the edit IS that diff. If
...
class TestPropertyPanelAudit(FrappeTestCase):
	def setUp(self):
	...
	def _versioning_on():
		...
		it always finds zero. update_element_properties calls doc.save() itself, so the
	...
	def test_an_edit_records_the_change_in_the_version_history(self):
		...
		with self._versioning_on():
			P.update_element_properties(self.model, "zz_task", {"assigneeMode": "User", "assigneeUser": "Administrator"})
	...
	def test_the_endpoint_does_not_report_a_log_it_no_longer_writes(self):
		out = P.update_element_properties(self.model, "zz_task", {"assigneeMode": "Round Robin"})
	...
	def test_an_unchanged_edit_writes_no_version(self):
		...
		with self._versioning_on():
			...
			out = P.update_element_properties(self.model, "zz_task", {"assigneeMode": "User", "assigneeUser": "Administrator"})
