...
Pure Python tests — Frappe dependencies are mocked.
"""
...
def _mock_deps():
	"""Mock Frappe and token dependencies."""
...
class TestResolveSubjectBody:
	...
	def test_email_template_fallback(self, mock_frappe):
		...
		subject, body = _resolve_subject_body(task_cfg, {}, "Task1", _make_instance())
		assert subject == "Template Subject"
		assert body == "<p>Template Body</p>"
	...
	def test_default_fallback(self, mock_frappe, mock_translate):
		...
		task_cfg = {}
		subject, body = _resolve_subject_body(task_cfg, {}, "Task1", _make_instance())
		assert "Task1" in subject or "INST-001" in subject
		assert "<p>" in body


class TestBuildOpenLink:
	...
	def test_with_context_doc(self, mock_frappe):
		...
		assert link == "https://erp.test.com/app/leave-application/HR-LAP-001"
	...
	def test_without_context_doc(self, mock_frappe):
		...
		assert link == "https://erp.test.com/app/bpmn-process-instance/INST-001"


class TestBuildActionsForEmail:
	...
	def test_simple_actions_get_tokens(self, mock_frappe):
		...
		assert "token" in actions[0]  # HMAC token present
		assert actions[0]["label"] == "Approve"
		assert actions[1]["label"] == "Reject"
	...
	def test_confirm_actions_get_links(self, mock_frappe):
		...
		assert "url" in actions[0]
		assert actions[0]["url"] == "https://erp.test.com/app/leave"
...
class TestSendEmail:
	...
	def test_sets_amp_flag(self, mock_frappe):
		...
		with patch.dict("sys.modules", {"one_fm": None, "one_fm.processor": None}):
			...
			assert mock_frappe.flags.amp_html == "<html amp>AMP</html>"
...
class TestComposeAndSendTaskEmail:
	...
	def test_sends_when_notify_true(self, mock_frappe, mock_ctx, mock_link, mock_send):
		...
		with patch("one_bpmn.email_builder.renderer.frappe", mock_frappe):
			with patch("one_bpmn.email_builder.renderer._get_template") as mock_tmpl:
				...
				assert args.kwargs.get("recipients") == ["user@test.com"] or args[1].get("recipients") == ["user@test.com"] or args[0][0] == ["user@test.com"]
