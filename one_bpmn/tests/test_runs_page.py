...
class RunsPageFixture(FrappeTestCase):
	...
	def setUpClass(cls):
		...
		instance = frappe.get_doc({"doctype": "BPMN Process Instance", "status": "Completed",
...
class TestStepRecording(RunsPageFixture):
	...
	def test_a_turn_with_its_own_window_keeps_it(self):
		...
		obs.record_selector_turns(run, [{
			"role": "assistant", "content": "done", "latency_ms": 3000,
			"started_at": start.isoformat(sep=" "), "ended_at": end.isoformat(sep=" "),
		}])
		row = frappe.get_all("AI Agent Step", filters={"run": run.name},
		                     fields=["started_at", "ended_at", "step_kind"])[0]
	...
	def test_step_kinds(self):
		...
		obs.record_ai_step(run, 3, "tool", "", tool_calls=[{"name": "lookup", "arguments": {}, "result": "{}"}])
		with obs.sub_call_scope(run, "review_script"):
			obs.record_sub_call("claude-sonnet-4-5", frappe._dict(text="ok", prompt_tokens=1, completion_tokens=1,
			                                                     cache_read_tokens=0, cache_write_tokens=0))
		obs.record_ai_step(run, 5, "assistant", "final")
		kinds = [r.step_kind for r in frappe.get_all(
...
class TestListRuns(RunsPageFixture):
	def test_lists_top_level_runs_with_counts_and_tiles(self):
		...
		obs.record_ai_step(parent, 2, "tool", "", tool_calls=[
			{"name": "a", "arguments": {}, "result": "Error calling a: boom", "status": "Error"},
			{"name": "b", "arguments": {}, "result": "{}"},
		], error_code="TOOL_ERROR", error_message="a: boom")
	...
	def test_errors_only_keeps_runs_with_a_failed_step_or_error_status(self):
		...
		names = [r.name for r in list_runs(instance=self.instance, origin="all", errors_only=1, page_length=50)["runs"]]
	...
	def test_filters_and_order(self):
		...
		only_fast = list_runs(instance=self.instance, origin="all", model="model-fast", page_length=50)["runs"]
		self.assertEqual([r.name for r in only_fast], [fast.name])
		self.assertEqual(list_runs(instance=self.instance, origin="all", user="nobody@example.com")["total"], 0)

	def test_search_matches_the_final_output(self):
		hit = self._run(final_output="the needle is here")
		self._run(final_output="hay")
		names = [r.name for r in list_runs(instance=self.instance, origin="all", search="needle", page_length=50)["runs"]]
	...
	def test_only_a_system_manager_may_list(self):
		user = frappe.db.get_value("User", {"name": ["not in", ["Administrator", "Guest"]], "enabled": 1}, "name")
		if not user:
			self.skipTest("no second user on this site")
		frappe.set_user(user)
		if "System Manager" in frappe.get_roles():
			self.skipTest("second user is a System Manager")
...
class TestRunDetail(RunsPageFixture):
	def test_detail_carries_instance_tree_and_siblings(self):
		...
		obs.record_ai_step(second, 2, "tool", "", tool_calls=[{"name": "lookup", "arguments": {"q": 1}, "result": "{}"}])
		...
		self.assertEqual([s["step_kind"] for s in detail["tree"]["steps"]], ["prompt", "tool_turn", "model_call"])
