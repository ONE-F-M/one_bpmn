...
def test_import_bpmn_returns_result(mock_frappe):
    ...
    doc.version = 1
    mock_frappe.get_doc.return_value = doc
    result = process_map_api.import_bpmn("<definitions />", "Imported")
...
def test_import_bpmn_sets_description_when_provided(mock_frappe):
    ...
    doc.version = 1
    mock_frappe.get_doc.return_value = doc
    process_map_api.import_bpmn("<definitions />", "Imported", "desc")
...
def test_import_bpmn_preserves_title(mock_frappe):
    ...
    doc.version = 1
    mock_frappe.get_doc.return_value = doc
    result = process_map_api.import_bpmn("<definitions />", "My Process")
...
def test_import_bpmn_uses_new_doc_path(mock_frappe):
    ...
    doc.version = 1
    mock_frappe.get_doc.return_value = doc
    process_map_api.import_bpmn("<definitions />", "Imported")
    assert mock_frappe.get_doc.called
