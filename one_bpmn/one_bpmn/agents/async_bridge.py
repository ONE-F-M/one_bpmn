...
own (and generally do not have) an asyncio event loop. The adapter ``stream()``
methods added in ONE-F-M/one_bpmn#823 are ``async def`` generators that yield
``StreamEvent`` objects inside an event loop.
