...
def get_assignment_rule_json_file(file_name: str, app_name: str = "one_bpmn"):
	"""
	Load JSON data from a file in the 'assignment_rule' folder.
	Args:
		file_name (str): The name of the JSON file (must end with '.json').
		app_name (str): The app name to resolve the path. Defaults to 'one_bpmn'.
	Returns:
		dict: The parsed JSON data.
	"""
	folder = frappe.get_app_path(app_name, "one_bpmn", "custom", "assignment_rule")
...
def create_assignment_rule(assignment_rule: dict, process_task_name: str = None):
	...
	Args:
		...
		process_task_name (str, optional): If provided, sets the custom_routine_task field.
