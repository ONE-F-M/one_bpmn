"""
Full-rip conversion (per-agent migration follow-up): the Docu chat agent's
pipeline stage tools no longer import the DocuAgent orchestrator. Each tool
Server Script in the process map's ad-hoc Tools sub-process now carries its own
self-contained, FLAT logic; the DocuAgent class in
one_bpmn.agents.google_adk.docu_agent.docu_agent has been deleted. This patch
installs those bodies idempotently so every environment converges on the
process-model-only Docu agent.
...
across agents: turn_state, the LLM adapter factory, get_agent_config, and the
Docu deterministic/read helpers (docu_agent.tools — validate_ir, diff_ir,
extract_json, read_doctype_definition, and the read ToolSpec bundles). tools.py
holds no orchestration/LLM/prompt logic (it is the same module the surviving
api/docu_api.py schema helpers import), so it stays as shared infra.
...
raw = run_sync(_adapter.complete(system=_system, user=prompt, tools=docu_tools.CLASSIFIER_TOOLS)).text
...
current_ir = docu_tools.read_doctype_definition(doctype) if exists else None
...
raw = run_sync(_adapter.complete(system=_system, user=prompt, tools=docu_tools.CLARIFIER_TOOLS)).text

question = raw or "Could you tell me a bit more?"
options = []
try:
    # extract_json tolerates markdown-fenced or prose-wrapped JSON.
...
if current_ir is None and doctype and bool(frappe.db.exists("DocType", doctype)):
    current_ir = docu_tools.read_doctype_definition(doctype)
...
draft = run_sync(_adapter.complete(system=_system, user=prompt, tools=docu_tools.WRITER_TOOLS)).text or ""
try:
...
if not draft_ir:
    ...
    review_raw = run_sync(_adapter.complete(system=_system, user=json.dumps(draft_ir), tools=docu_tools.REVIEWER_TOOLS)).text
    ...
    validation = docu_tools.validate_ir(candidate)
    if validation["valid"]:
        update_turn(
