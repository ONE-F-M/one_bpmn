...
_SCHEMA_WRITER = (
	...
	"3. Never add a NEW field with a Frappe built-in name (name, owner, creation, modified, modified_by, docstatus, parent, parentfield, parenttype, idx, amended_from, _assign, _comments) — they exist automatically. A built-in the DocType you are MODIFYING already carries (amended_from on a submittable one, say) is not new: keep it exactly where it is, per rule 7.\n"
...
_SCHEMA_REVIEWER = (
	...
	"2. Fieldnames — snake_case and unique, and no NEW field takes a reserved Frappe name (name, owner, creation, modified, modified_by, docstatus, parent, parentfield, parenttype, idx, amended_from, _assign, _comments). A reserved field the DocType already carries is not new — leave it exactly as it is; dropping it is a blocking error.\n"
