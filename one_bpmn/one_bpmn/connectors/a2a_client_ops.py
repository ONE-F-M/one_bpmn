...
def list_delegatable_agents(params: dict, ctx: dict) -> dict:
	"""The directory: which agents this agent may hand work to, and what each
	is for (WI-001933).

	The point is that a delegating agent should not need its targets written
	into the diagram. Give it this as a tool and it can look up the roster at
	run time, read what each agent is good at, then call the delegate tool
	with the one it picked — the same shape a person gets from the A2A page,
	because it is the same card data.

	Scoped to what this agent could actually reach: exposed + Live, and if the
	agent restricts delegation, only the agents it lists. An agent must never
	read about a specialist it would then be refused.
	"""
	from one_bpmn.agents.a2a.card import build_agent_card

	delegating_agent = _delegating_agent(ctx.get("instance"), params)
	roster = []
	for agent in frappe.get_all(
		"AI Agent Configuration",
		filters={"enabled": 1, "lifecycle_status": "Live", "a2a_exposed": 1},
		fields=["name", "agent_id", "agent_name"],
		order_by="agent_name asc",
	):
		if agent.name == delegating_agent:
			continue  # nobody needs to be told they can delegate to themselves
		if delegating_agent and not guardrails.may_delegate_to(delegating_agent, agent.name):
			continue
		...
		roster.append(
			{
				# `agent` is the exact value the delegate tool wants back.
				"agent": agent.name,
				"name": card.get("name") or agent.agent_name,
	...
	return {
		...
		"count": len(roster),
		"note": (
			"Pick one and pass its 'agent' value to the delegate tool. An empty list means "
			"there is nobody to delegate to — do the work yourself or say so."
		),
	}
