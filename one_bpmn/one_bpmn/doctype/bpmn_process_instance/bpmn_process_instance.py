...
class BPMNProcessInstance(Document):
	...
	def _run_engine(self, wf):
		...
		"""
		# WI-001359: audit every ad-hoc inner-task activation while this
		# engine call runs. The hook is process-global engine state —
		# install/uninstall it around the run.
		bpmn_engine.adhoc_task_activated_logger = self._log_adhoc_activation
		try:
			self._run_engine_body(wf)
		finally:
