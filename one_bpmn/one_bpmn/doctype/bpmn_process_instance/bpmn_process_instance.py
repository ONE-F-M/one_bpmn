...
class BPMNProcessInstance(Document):
    ...
    def start(self, initial_data: dict = None):
        ...
        if not model.serialized_spec:
            frappe.throw(
                _(
                    'Process model "{0}" has no compiled spec. '
                    'Call compile_process_model() first.'
    ...
    def advance(self, task_id: str, data: dict = None) -> list:
        ...
        if self.status in ('Completed', 'Cancelled'):
            frappe.throw(
                _('Instance "{0}" is already {1} and cannot be advanced.').format(
    ...
    def _apply_frappe_workflow_action(self, action: str) -> None:
        ...
        if not (self.context_doctype and self.context_docname):
            frappe.throw(
                _('Cannot apply Frappe workflow: no context document linked to this instance.')
            )
    ...
    def _dispatch_service_task(self, task):
        ...
        if service_type == 'apply_workflow':
            ...
            doctype = (
                task_cfg.get('serviceTargetDoctype')
                or self.context_doctype
            )
    ...
    def _sync_active_tasks(self, wf):
        ...
        self.active_tasks = [
            ...
            if row.status == 'Completed' or row.task_id in new_ready_ids
        ]

        existing_waiting_ids = {
            row.task_id for row in self.active_tasks if row.status == 'Waiting'
        }
