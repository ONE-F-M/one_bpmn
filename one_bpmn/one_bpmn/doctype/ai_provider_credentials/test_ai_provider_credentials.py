...
"""
Tests for the AI Provider Credentials doctype.

Covers:
  (a) Admin can create an AI Provider Credentials with all required fields
  (b) as_dict() does NOT contain the api_key value
  (c) doc.get_password("api_key") returns the real key
  (d) frappe.get_list("AI Provider Credentials") does NOT include api_key in results
  (e) A non-admin user cannot frappe.get_doc an AI Provider Credentials (PermissionError)
...
def make_ai_provider(**kwargs) -> Document:
    """Factory function for AI Provider Credentials test fixtures."""
...
class TestAIProviderCredentials(FrappeTestCase):
    def test_create_ai_provider(self):
        doc = make_ai_provider()
        self.assertTrue(frappe.db.exists("AI Provider Credentials", doc.name))

    def test_api_key_not_in_as_dict(self):
        doc = make_ai_provider()
        loaded = frappe.get_doc("AI Provider Credentials", doc.name)
        d = loaded.as_dict()
    ...
    def test_non_admin_cannot_read(self):
        ...
        try:
            with self.assertRaises(frappe.PermissionError):
                frappe.get_doc("AI Provider Credentials", doc.name)
        finally:
