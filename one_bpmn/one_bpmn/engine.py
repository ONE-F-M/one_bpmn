...
from SpiffWorkflow.bpmn.specs.defaults import (
    UserTask, ScriptTask, ServiceTask, ManualTask, SendTask,
...
class FrappeScriptEngine(PythonScriptEngine):
    ...
    Gateway conditions can then check: result["decision"] == "Approve"
    ...
    def __init__(self, environment, script_task_extensions=None,
                 context_doctype=None, context_docname=None):
    ...
    def _run_frappe_server_script(self, script_name: str, task) -> None:
        ...
        via conditions like: decision == "Approve"
        """
        try:
        ...
        if self._context_doctype and self._context_docname:
            try:
                local_vars['doc'] = _frappe.get_doc(
                    self._context_doctype, self._context_docname
                )
            except Exception:
...
def get_task_type_label(task) -> str:
    ...
    spec = task.task_spec
    if isinstance(spec, UserTask):       return 'User Task'
    if isinstance(spec, ScriptTask):     return 'Script Task'
    if isinstance(spec, ServiceTask):    return 'Service Task'
    if isinstance(spec, ManualTask):     return 'Manual Task'
    if isinstance(spec, SendTask):       return 'Send Task'
    if isinstance(spec, ExclusiveGateway): return 'Exclusive Gateway'
    if isinstance(spec, ParallelGateway):  return 'Parallel Gateway'
    if isinstance(spec, InclusiveGateway): return 'Inclusive Gateway'
