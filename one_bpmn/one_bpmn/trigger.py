...
def _find_matching_models(doctype: str, trigger_event: str) -> list:
    ...
    via_child = frappe.db.sql(
        """
        SELECT DISTINCT pm.name
        FROM   `tabBPMN Process Model`  pm
        JOIN   `tabBPMN Process DocType` pd
               ON pd.parent     = pm.name
               AND pd.parenttype = 'BPMN Process Model'
        WHERE  pm.is_active      = 1
          AND  pm.trigger_type   = 'DocType Event'
          AND  pm.trigger_event  = %(event)s
          AND  pd.doctype_name   = %(doctype)s
        """,
        {'event': trigger_event, 'doctype': doctype},
        as_dict=False,
    )
    via_child_names = [row[0] for row in via_child]
...
def _maybe_start_instance(doc, model_name: str):
    ...
    frappe.logger('one_bpmn').info(
        f'BPMN instance {instance.name} started for '
        f'{doc.doctype}/{doc.name} via model {model_name}'
    )
