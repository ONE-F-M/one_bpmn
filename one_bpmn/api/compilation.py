...
def _validate_ai_tool_contract(service_extensions: dict) -> None:
	...
	for agent_id, cfg in agents.items():
		...
		# linked shape keeps a stale copy that nothing reads, and blocking on
		# text no screen shows is worse than not checking it. The turn's own
		# instructions name tools the same way, so they count too.
		instructions = "\n".join(
