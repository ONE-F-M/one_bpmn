...
# ============================================
# Server Script API (Pre-Script / Post-Script)
# ============================================

@frappe.whitelist()
def list_server_scripts() -> list:
	"""
	List all Server Script records for the Pre/Post-Script selector.

	Returns:
		list of server script summaries
	"""
...
def create_server_script(
	...
	"""
	Create a new Server Script record.

	Args:
		script_name: Name for the Server Script (used as the document name)
		script_type: One of 'DocType Event', 'Scheduler Event', 'Permission Query', 'API'
		script: Python script content
		reference_doctype: Required for DocType Event / Permission Query
		doctype_event: Required for DocType Event
		api_method: Required for API type
		allow_guest: Whether to allow guest access (API type only)
		event_frequency: Required for Scheduler Event
		cron_format: Required when event_frequency is 'Cron'
		module: Optional module for export

	Returns:
		dict with name and script_type of the created script
	"""
	if not script_name or not script_type or not script:
		frappe.throw(_("Script name, type, and content are required"))

	valid_types = ["DocType Event", "Scheduler Event", "Permission Query", "API"]
	if script_type not in valid_types:
		frappe.throw(_("Invalid script type. Must be one of: {0}").format(", ".join(valid_types)))
