...
@frappe.whitelist()
def search_processes_for_lucrusher(search_term: str) -> dict:
	"""
	Search Process records by name and process_name for the LuCrusher agent.

	Called by LuCrusher on the Processa production system to find existing
	process records matching a user-supplied search term.

	Returns a dict with:
	  - exact_match:  single Process dict when the term matches exactly (or None)
	  - partial_matches: list of Process dicts for partial/similar matches
	  - total: total number of results found

	Security:
	  - Uses frappe.get_list which enforces read permissions for the calling user.
	"""
	if not search_term or not search_term.strip():
		return {"exact_match": None, "partial_matches": [], "total": 0}

	term = search_term.strip()

	# 1. Try exact match on `name` (document ID)
	exact = None
	if frappe.db.exists("Process", term):
		doc = frappe.db.get_value(
			"Process",
			term,
			["name", "process_name", "process_owner", "process_owner_name", "description"],
			as_dict=True,
		)
		if doc:
			exact = doc

	# 2. Try exact match on `process_name` field (human-readable title)
	if not exact:
		exact_by_title = frappe.db.get_value(
			"Process",
			{"process_name": term},
			["name", "process_name", "process_owner", "process_owner_name", "description"],
			as_dict=True,
		...
		if exact_by_title:
			exact = exact_by_title

	# 3. Partial / fuzzy search across name and process_name
	partial_results = frappe.get_list(
		"Process",
		or_filters=[
			["name", "like", f"%{term}%"],
			["process_name", "like", f"%{term}%"],
		],
		fields=["name", "process_name", "process_owner", "process_owner_name", "description"],
		order_by="process_name asc",
		limit=20,
	)

	# Remove exact match from partial list to avoid duplication
	if exact:
		partial_results = [p for p in partial_results if p["name"] != exact["name"]]

	return {
		"exact_match": exact,
		"partial_matches": partial_results,
		"total": (1 if exact else 0) + len(partial_results),
	}
